...
import { lexusData } from '@/data/cars/lexus';
import { QuoteForm } from '@/components';
import Link from 'next/link';

export default function LexusWreckersPage() {
  // Calculate total range across all models
  const modelYears = Object.values(lexusData.models).map(model => {
  ...
  const allPopularParts = Array.from(
    new Set(
      Object.values(lexusData.models)
  ...
  return (
    <main>
      <EnhancedMakeModelHero
        ...
        manufacturingHistory={lexusData.description}
      />

      <div className="bg-white py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Lexus Models We Buy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(lexusData.models).map(([slug, model]) => (
              <Link
                href={`/cars/lexus/${slug}`}
                key={slug}
