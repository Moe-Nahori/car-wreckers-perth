...
export default function HoldenWreckersPage() {
  // Calculate total range across all models
  const modelYears = Object.values(holdenData.models).map(model => {
    const [start] = model.yearRange.split('-');
    return parseInt(start);
  });
  const earliestYear = Math.min(...modelYears);
  const yearRange = `${earliestYear}-2020`; // Holden ceased operations in 2020

  // Get all popular parts across models
  const allPopularParts = Array.from(
    new Set(
      Object.values(holdenData.models)
  ...
  return (
    <main>
      <EnhancedMakeModelHero
        ...
        manufacturingHistory={holdenData.description}
      />

      <div className="bg-white py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Holden Models We Buy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(holdenData.models).map(([slug, model]) => (
              <Link
                href={`/cars/holden/${slug}`}
                key={slug}
