...
export default function MazdaWreckersPage() {
  // Calculate total range across all models
  const modelYears = Object.values(mazdaData.models).map(model => {
  ...
  const allPopularParts = Array.from(
    new Set(
      Object.values(mazdaData.models)
  ...
  return (
    <main>
      <EnhancedMakeModelHero
        ...
        manufacturingHistory={mazdaData.description}
      />

      <div className="bg-white py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Mazda Models We Buy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(mazdaData.models).map(([slug, model]) => (
              <Link
                href={`/cars/mazda/${slug}`}
                key={slug}
