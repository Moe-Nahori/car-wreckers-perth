...
export default function ToyotaWreckersPage() {
  // Calculate total range across all models
  const modelYears = Object.values(toyotaData.models).map(model => {
  ...
  const allPopularParts = Array.from(
    new Set(
      Object.values(toyotaData.models)
  ...
  return (
    <main>
      <EnhancedMakeModelHero
        ...
        manufacturingHistory={toyotaData.description}
      />

      <div className="bg-white py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Toyota Models We Buy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(toyotaData.models).map(([slug, model]) => (
              <Link
                href={`/cars/toyota/${slug}`}
                key={slug}
