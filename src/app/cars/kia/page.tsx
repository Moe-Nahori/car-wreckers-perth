...
export default function KiaWreckersPage() {
  // Calculate total range across all models
  const modelYears = Object.values(kiaData.models).map(model => {
  ...
  return (
    <main>
      <EnhancedMakeModelHero
        ...
        manufacturingHistory={kiaData.description}
      />

      <div className="bg-white py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Kia Models We Buy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(kiaData.models).map(([slug, model]) => (
              <Link
                href={`/cars/kia/${slug}`}
                key={slug}
