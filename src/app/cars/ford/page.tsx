...
export default function FordWreckersPage() {
  // Calculate total range across all models
  const modelYears = Object.values(fordData.models).map(model => {
    ...
    <main>
      ...
      <div className="bg-white py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Ford Models We Buy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(fordData.models).map(([slug, model]) => (
              <Link
                href={`/cars/ford/${slug}`}
                key={slug}
