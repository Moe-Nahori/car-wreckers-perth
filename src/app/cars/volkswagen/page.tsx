...
export const metadata: Metadata = {
  title: 'Volkswagen Wreckers Perth | Cash for VW Cars',
  description: 'Get top cash for your Volkswagen vehicle in Perth. We buy all VW models including Golf, Polo, Tiguan, Passat, and Amarok. Free quotes and same-day removal.',
};

export default function VolkswagenPage() {
  // Calculate total range across all models
  const modelYears = Object.values(volkswagenData.models).map(model => {
  ...
  const allPopularParts = Array.from(
    new Set(
      Object.values(volkswagenData.models)
  ...
  return (
    <main>
      <EnhancedMakeModelHero
        ...
        manufacturingHistory={volkswagenData.description}
      />

      <div className="bg-white py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Volkswagen Models We Buy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(volkswagenData.models).map(([slug, model]) => (
              <Link
                href={`/cars/volkswagen/${slug}`}
                key={slug}
